...
<section class="border border-slate-300 bg-white shadow-sm">
                        ...
                        <tr>
                            ...
                            @for ($bidNumber = 1; $bidNumber <= 3; $bidNumber++) @php $savedVendorName=$savedBids[$item->id][$bidNumber]['vendor_name'] ?? '';
                                ...
                                <td class="relative align-top border border-slate-300 p-0">
                                    <input type="text" name="bids[{{ $item->id }}][{{ $bidNumber }}][vendor_name]" value="{{ $vendorNameValue }}" placeholder="Vendor {{ $bidNumber }}" autocomplete="off" spellcheck="false" class="js-vendor-search h-11 w-full border-0 px-3 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-blue-100">
                                ...
                                <td class="align-top border border-slate-300 p-0">
                                    <input type="text" name="bids[{{ $item->id }}][{{ $bidNumber }}][unit_price]" value="{{ $unitPriceValue }}" inputmode="decimal" autocomplete="off" placeholder="0" class="h-11 w-full border-0 px-3 text-right text-sm text-slate-900 outline-none focus:ring-2 focus:ring-blue-100">
                                </td>
                                @endfor
                        </tr>
                        @empty
    ...
    <div class="flex flex-col justify-end gap-3 border-t border-slate-300 p-5 md:flex-row">
        <button type="button" data-open-modal="send-back-requester-modal" class="inline-flex h-10 items-center justify-center border border-blue-700 bg-white px-6 text-sm font-bold text-blue-800 transition hover:bg-blue-50">
        ...
        <form method="POST" action="{{ route('purchasing-lite.purchase-requests.vendors.send-to-cost-control', $purchaseRequest) }}" onsubmit="return confirm('Send this PR to Cost Control? Please make sure vendor bids have been saved.');">
            @csrf

            <button type="submit" class="inline-flex h-10 w-full items-center justify-center bg-green-700 px-6 text-sm font-bold text-white transition hover:bg-green-800 md:w-auto">
                Send to Cost Control
            </button>
        </form>

        <button type="submit" form="vendor-bids-form" class="inline-flex h-10 items-center justify-center bg-slate-950 px-6 text-sm font-bold text-white transition hover:bg-slate-800">
            Save Bids
        </button>
    </div>
</section>
